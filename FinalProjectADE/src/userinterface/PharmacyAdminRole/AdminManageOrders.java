...
public class AdminManageOrders extends javax.swing.JPanel {
    ...
    public AdminManageOrders(JPanel userProcessContainer, Pharmacy pharmacy) {
        ...
        System.out.println(pharmacy.getDeliveryDirectory().getDeliveryList().size());
        for(Delivery dm : pharmacy.getDeliveryDirectory().getDeliveryList())
        {
            System.out.println(dm.getDeliveryName());
            comboDeliveryAgent.addItem(dm.getDeliveryName());
        }
    }
    ...
    private void btnAssignDeliveryAgentActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAssignDeliveryAgentActionPerformed
        ...
        if (selectedRow >= 0)
        {
            ...
            if(comboDeliveryAgent.getSelectedItem() != "")
            {
                ...
                Delivery dm = pharmacy.findDelivery(dmName);
                order.setDelivery(dm);
    ...
    private void populateTable() {
       DefaultTableModel dtm = (DefaultTableModel)tblOrders.getModel();
        ...
        {
            for(Order orders : pharmacy.getOrderDirectory().getOrderList())
            {
                if(!orders.isStatus())
                {
                    ...
                    row[0] = orders;
                    try {
                        row[1] = orders.getPatient().getName();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                    row[2] = orders.getDate();
                    ...
                    {
                        row[3] = orders.getDelivery().getDeliveryName();
                    }
                    dtm.addRow(row);
                }
            }
