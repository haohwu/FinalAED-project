...
public class HospitalOrderAction extends javax.swing.JPanel{
    ...
    private void HOAOrderButtonActionPerformed(java.awt.event.ActionEvent evt) {                                               
        ...
        order = pharma.getOrderDirectory().createHospitalOrder(hospital);
        for(Medicine mo : cart)
        {
            order.addItem(mo);
        }
        order.calculateTotalAmount();
        JOptionPane.showMessageDialog(null, "Thank you for your order! Order of " + order.getMedicineList().size() + " medicine item(s) for amount $" + order.getAmount()+ " is placed !");
    }
    ...
    private void AddHospMedicineItemButtonActionPerformed(java.awt.event.ActionEvent evt) {                                                          
        ...
        if (selectedRow >= 0)
        {
            Medicine mi2 = (Medicine) HospMedicineTable.getValueAt(selectedRow, 1);
    ...
    private void HOADeleteItemButtonActionPerformed(java.awt.event.ActionEvent evt) {                                                    
        ...
        if (selectedRow1 >= 0)
        {
            Medicine mi1 = (Medicine) HOAOrderTable.getValueAt(selectedRow1, 1);
            cart.remove(mi1);
