 ...
 */
package SupplierDeliveryManRole;
...
public class SupplierDeliveryManWorkArea extends javax.swing.JPanel {
     ...
     */
    public SupplierDeliveryManWorkArea(JPanel userProcessContainer, User user, EcoSystem business) {
        initComponents();
         this.userProcessContainer = userProcessContainer;
        this.user = user;
        this.business = business;
        
        for(SupplierManager s : business.getSupplierManagerDirectory().getSupplierManagersList())
        {
            if(s.findSupplierDelivery(user.getEmployee().getName()) != null)
            {
                this.supplierDelivery = s.findSupplierDelivery(user.getEmployee().getName());
                this.supplierManager = s;
                break;
            }
        }
        lblDeliveryAgent.setText("Orders to be delivered by " + this.supplierDelivery);
        
            populateTable();
    }    
        public void populateTable(){
        if(supplierManager.getSupplierOrderDirectory() != null)
        {
            DefaultTableModel dtm = (DefaultTableModel)tblWorkRequest.getModel();
            dtm.setRowCount(0);
            for(SupplierOrder so : supplierManager.getSupplierOrderDirectory().getSupplierOrderList())
            {
                if(so.getSupplierDelivery().equals(supplierDelivery))
                {
                    Object[] row = new Object[dtm.getColumnCount()];
                    row[0] = so;
                    row[1] = so.getAmount();
                    row[2] = so.getDetail();
                    row[3] = so.getSupplierDelivery();
                    if(so.isOrderStatus())
                    {
                        row[4] = "Yes";
                    }
    ...
    private void btnProcessActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProcessActionPerformed
        int selectedRow = tblWorkRequest.getSelectedRow();
              if (selectedRow >= 0)
              {
                  ...
                  {
                      SupplierOrder so = (SupplierOrder) tblWorkRequest.getValueAt(selectedRow, 0);
