...
public class AdminManageSupplierOrders extends javax.swing.JPanel {
    ...
    public AdminManageSupplierOrders(JPanel userProcessContainer, SupplierManager supplierManager) {
        initComponents();
        ...
        System.out.println(supplierManager.getSupplierDeliveryDirectroy().getSupplierDeliverylist().size());
        for(SupplierDelivery suoDelivery : supplierManager.getSupplierDeliveryDirectroy().getSupplierDeliverylist())
        {
            System.out.println(suoDelivery.getSupplierDeliveryName());
            comboSupplierDeliveryAgent.addItem(suoDelivery.getSupplierDeliveryName());
        }
    }
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
       ...
       int selectedRow = tblOrders.getSelectedRow();
        if (selectedRow >= 0)
        {
            SupplierOrder supplierOrders = (SupplierOrder) tblOrders.getValueAt(selectedRow, 0);
            supplierManager.deleteOrder(supplierOrders);
    ...
    private void btnAssignDeliveryAgentActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAssignDeliveryAgentActionPerformed
        ...
        if (selectedRow >= 0)
        {
            ...
            if(comboSupplierDeliveryAgent.getSelectedItem() != "")
            {
                ...
                SupplierDelivery sdm = supplierManager.findSupplierDelivery(dmName);
                supplierOrders.setSupplierDelivery(sdm);
...
private void populateTable() {
       DefaultTableModel dtm = (DefaultTableModel)tblOrders.getModel();
        dtm.setRowCount(0);
        if(supplierManager.getSupplierOrderDirectory().getSupplierOrderList() != null)
        {
            for(SupplierOrder supplierOrders : supplierManager.getSupplierOrderDirectory().getSupplierOrderList())
            {
                if(!supplierOrders.isOrderStatus()&& supplierOrders.getPharmacy()!= null)
                {
                    Object[] row = new Object[dtm.getColumnCount()];
                    row[0] = supplierOrders;
                    row[1] = supplierOrders.getPharmacy().getPharmacyName();
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                    row[2] = supplierOrders.getOrderDate();
                    if(supplierOrders.getSupplierDelivery()== null)
                    {
                        row[3] = "";
                    }
                    else
                    {
                        row[3] = supplierOrders.getSupplierDelivery().getSupplierDeliveryName();
                    }
                    dtm.addRow(row);
                }
            }
